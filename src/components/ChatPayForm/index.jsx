...
import Tab from '../Tab';
import CardUnregistered from '../CardUnregistered';
...
function ChatPayForm({ cardInfo }) {
    const navigate = useNavigate();

    // 카드 등록 여부
    const hasCardInfo = cardInfo !== null;
    ...
    // 쿠폰
    const defaultCheckCoupon = useMemo(() => hasCardInfo ? 'coupon1' : '', [hasCardInfo]);
    const [checkCoupon, handlerCoupon] = useInputRadio(defaultCheckCoupon, true);
    ...
    // 결제 금액
    const [onceAmount] = useState(14900);
    ...
    // 결제 수단 탭
    const methodDefaultIndex = 0;
    const [methodTab, setMethodTab] = useState(methodDefaultIndex);
    const onChangeMethodTab = useCallback((index) => {
        setMethodTab(index);
    }, []);

    // 결제 수단 - 등록 카드 변경 모달
    const [openCardChangeModal, setOpenCardChangeModal] = useState(false);
    const onOpenCardChangeModal = useCallback(() => {
        setOpenCardChangeModal(true);
    }, []);
    const onCloseCardChangeModal = useCallback(() => {
        setOpenCardChangeModal(false);
    }, []);
    const onClickCardChange = useCallback(() => {
        navigate('/card/change');
    }, [navigate]);
    ...
    useEffect(() => {
        if (methodTab === 0 && !hasCardInfo) return;
        setCanSubmit(true);
        
        return () => {
            setCanSubmit(false);
        }
    }, [methodTab, hasCardInfo]);        
        ...
        <ChatPayForms onSubmit={onSubmitPay}>
            ...
            <article className='form-method'>
                <FieldTitles mb={8}>
                    <div className='title'>
                        <h6>결제 수단</h6>
                        {hasCardInfo && <button type='button' className='type2' onClick={onOpenCardChangeModal}><b>등록 카드 변경</b></button>}
                        {hasCardInfo && openCardChangeModal &&
                            <Modal title={'카드 변경 시 기존 등록된 카드는 삭제됩니다.'} text={'계속 하시겠습니까?'}>
                                <button type='button' onClick={onCloseCardChangeModal}>아니오</button>
                                <button type='button' onClick={onClickCardChange}>네</button>
                            </Modal>
                        }
                    </div>
                </FieldTitles>
                <Tab labels={['간편결제', '일반결제']} defaultIndex={methodDefaultIndex} handler={onChangeMethodTab} tabType={2} />
                {methodTab === 0 && 
                    <div className='form-method-tab-con'>
                        {hasCardInfo
                            ? <CardRegistered cardInfo={cardInfo} canDelete={false} />
                            : <CardUnregistered title={'지금만 가능해요!'} title2={'카드 등록하고 쿠폰 즉시 적용하기'} text={'커피챗 대화 연장 기능은 카드 등록 후에만 가능합니다.'} />
                        }
                    </div>
                }
                {methodTab === 1 &&           
                    <div className='form-method-tab-con'>
                        <div className='form-method-recommend'>
                            <div className='form-method-recommend-text'>
                                <p><b>잠깐! 20분은 정말 짧아요!</b></p>
                                <p>커피챗 대화 연장기능 사용을 위해 카드를 미리 등록해 주세요.</p>
                            </div>
                            <Link to='/card/register' className='form-method-recommend-link'>카드 등록 하기</Link>
                        </div>
            ...
            <div className='form-submit'>
                <Buttons type='submit' disabled={!canSubmit}>확인 및 결제하기</Buttons>
            </div>
        </ChatPayForms>
    );
}

ChatPayForm.propTypes = {
